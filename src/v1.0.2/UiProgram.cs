...
internal sealed class MainForm : Form
{
    ...
    private const string MonitorExe = "napcat-monitor-v1.0.2.exe";
    ...
    private TabPage BuildConfigPage()
    {
        ...
        _expectedWidth = Number(1, 10000);
        AddRow(panel, "窗口宽度", _expectedWidth);

        _expectedHeight = Number(1, 10000);
    ...
    private TabPage BuildClickPage()
    {
        ...
        var buttons = new FlowLayoutPanel { Dock = DockStyle.Fill };
        buttons.Controls.Add(Button("保存点击配置", SaveConfigFromUi));
    ...
    {
        var button = new Button { Text = text, Width = 120, Height = 32, Margin = new Padding(4) };
    ...
    private void SaveNapCatConfig()
    {
        ...
        string napcatConfigDir = Path.Combine(desktopRoot, "components", "NapCatQQ", "config");
        if (!Directory.Exists(napcatConfigDir))
        {
            MessageBox.Show("未找到 NapCat 配置目录，请先安装 NapCat。", "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
            return;
        }
        ...
        try
        {
            ...
            MessageBox.Show("NapCat 配置已写入。下一步启动 NapCat，并按界面完成 QQ 登录。", "完成", MessageBoxButtons.OK, MessageBoxIcon.Information);
            RefreshStatus();
        }
        catch (Exception ex)
        {
    ...
    private void RunTool(string args, bool captureOutput)
    {
        ...
        if (!captureOutput)
        {
            Process.Start(new ProcessStartInfo(exe, args) { WorkingDirectory = _root, UseShellExecute = true });
            return;
        }
