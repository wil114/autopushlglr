...
internal sealed class MainForm : Form
{
    ...
    private const string MonitorExe = "napcat-monitor-v1.0.3.exe";
    ...
    private void RunTool(string args, bool captureOutput)
    {
        ...
        if (!captureOutput)
        {
            string command = "/k \"\"" + exe + "\" " + args + "\"";
            Process.Start(new ProcessStartInfo("cmd.exe", command) { WorkingDirectory = _root, UseShellExecute = true });
            return;
        }
