...
{
    private const string ProgramVersion = "v1.0.6";
    ...
    private static string _runtimeState = "Idle";
    ...
    private static int Main(string[] args)
    {
        ...
        try
        {
            ...
            if (command == "--detect-resolution" && args.Length > 1)
                config.game_process_name = args[1];
            ValidateConfig(config, command.Length == 0);

            if (command == "--detect-resolution")
                return DetectResolution(config);
            if (command == "--capture-points")
                return CapturePoints(config);
            if (command == "--dry-run-clicks")
                return DryRunClicks(config);
            if (command == "--preview-captured-points")
                return PreviewCapturedPoints(config);
            if (command == "--test-captured-clicks")
                return TestCapturedClicks(config);
            if (command == "--edit-captured-delays")
                return EditCapturedDelays();
            if (command == "--clear-captured-points")
    ...
    private static void ValidateConfig(Config config, bool requireMonitor)
    {
        ...
        if (config.keywords == null) config.keywords = new string[0];
        ...
        {
            if (requireMonitor) throw new InvalidDataException("keyword_match 只能是 any 或 all。");
            config.keyword_match = "any";
        }

        if (requireMonitor && !String.IsNullOrWhiteSpace(config.callback_url))
        {
            Uri callbackUri;
        ...
        if (requireMonitor && (config.callback_timeout_ms < 200 || config.callback_timeout_ms > 30000))
            throw new InvalidDataException("callback_timeout_ms 必须在 200 到 30000 之间。");
        if (requireMonitor && (config.reconnect_seconds < 1 || config.reconnect_seconds > 60))
            throw new InvalidDataException("reconnect_seconds 必须在 1 到 60 之间。");
        if (requireMonitor && (config.dedupe_cache_size < 100 || config.dedupe_cache_size > 100000))
            throw new InvalidDataException("dedupe_cache_size 必须在 100 到 100000 之间。");
        if (!config.trigger_cooldown_ms.HasValue)
            config.trigger_cooldown_ms = DefaultTriggerCooldownMs;
        if (config.trigger_cooldown_ms.Value < 0 || config.trigger_cooldown_ms.Value > 600000)
            throw new InvalidDataException("trigger_cooldown_ms 必须在 0 到 600000 之间。");
        ...
        for (int i = 0; i < config.click_steps.Length; i++)
        {
            ...
            if (requireMonitor && config.automation_enabled && (step.x < 0 || step.y < 0))
                throw new InvalidDataException(step.name + " 尚未配置有效坐标。");
        }
        if (requireMonitor && config.automation_enabled)
        {
    ...
    private static async Task<int> Run(Config config)
    {
        while (!Stop.IsCancellationRequested)
        {
            using (ClientWebSocket socket = new ClientWebSocket())
            {
                ...
                try
                {
                    SetRuntimeState("WaitingNapCat", "正在连接 " + config.napcat_ws_url);
                    ...
                    SetRuntimeState("Listening", "已连接 NapCat，等待关键词。");
                    Log("已连接 NapCat，开始接收群消息事件。");
                    await ReceiveLoop(socket, config);
                    if (!Stop.IsCancellationRequested)
                        SetRuntimeState("WaitingNapCat", "NapCat 连接关闭，准备重连。");
                    if (!Stop.IsCancellationRequested)
                ...
                {
                    SetRuntimeState("WaitingNapCat", "连接失败，等待重试：" + ex.Message);
    ...
    private static void ProcessEvent(Config config, string eventJson)
    {
        try
        {
            ...
            SetRuntimeState("HitReceived", "命中：" + Truncate(hit.Content, 80));
    ...
    private static void QueueAutomation(Config config, Hit hit)
    {
        if (!config.automation_enabled)
        {
            ...
            SetRuntimeState("Listening", "命中已记录，自动点击关闭。");
            return;
        }
        ...
        lock (StateLock)
        {
            if (_automationRunning != 0)
            {
                Log("状态机：当前正在点击，本次命中已跳过。");
                return;
            }
            if (now < _cooldownUntilUtc)
            {
                Log("状态机：当前处于冷却中，剩余 " + (int)(_cooldownUntilUtc - now).TotalMilliseconds + "ms，本次命中已跳过。");
                return;
            }
            _automationRunning = 1;
            _cooldownUntilUtc = DateTime.MinValue;
        }
        SetRuntimeState("Clicking", "开始处理命中：" + Truncate(hit.Content, 80));

        ThreadPool.QueueUserWorkItem(delegate
        {
            bool success = false;
            try { success = ExecuteClicks(config, hit); }
            catch (Exception ex) { Log("自动点击失败：" + ex.Message); }
            finally { FinishAutomation(config, success); }
        });
    }

    private static void FinishAutomation(Config config, bool success)
    {
        int cooldownMs = config.trigger_cooldown_ms.GetValueOrDefault(DefaultTriggerCooldownMs);
        if (cooldownMs > 0 && !Stop.IsCancellationRequested)
        {
            lock (StateLock)
            {
                _automationRunning = 0;
                _cooldownUntilUtc = DateTime.UtcNow.AddMilliseconds(cooldownMs);
            }
            SetRuntimeState("Cooldown", (success ? "点击完成" : "点击失败") + "，冷却 " + cooldownMs + "ms。");
            ThreadPool.QueueUserWorkItem(delegate
            {
                ...
                bool canReturn;
                lock (StateLock)
                    canReturn = _automationRunning == 0 && _runtimeState == "Cooldown" && DateTime.UtcNow >= _cooldownUntilUtc;
                if (canReturn) SetRuntimeState("Listening", "冷却结束，继续监听。");
            });
            return;
        }
        ...
        SetRuntimeState(Stop.IsCancellationRequested ? "Stopped" : "Listening", success ? "点击完成，继续监听。" : "点击失败，继续监听。");
    }
    ...
    private static int CapturePoints(Config config)
    {
        ...
        if (File.Exists(path))
        {
            ...
            if (HasClickSteps(previous))
            {
                ...
                Console.WriteLine("坐标文件没有已采集步骤，将从步骤1开始。");
        }
    ...
    private static int DetectResolution(Config config)
    {
        IntPtr window = FindGameWindow(config.game_process_name);
        if (window == IntPtr.Zero)
        {
        ...
        Console.WriteLine("{0}x{1}", client.Right - client.Left, client.Bottom - client.Top);
        return 0;
    }
    ...
    private static bool HasClickSteps(Config config)
    {
        return config != null && config.click_steps != null && config.click_steps.Length > 0;
    }
    ...
    {
        return Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "日志", "运行状态.json");
    }

    private static void SetRuntimeState(string state, string detail)
    {
        string previous;
        ...
        lock (StateLock)
        {
            ...
            _runtimeState = state;
            cooldownUntil = _cooldownUntilUtc;
        }

        Dictionary<string, object> payload = new Dictionary<string, object>();
        payload["version"] = ProgramVersion;
        payload["state"] = state;
        payload["detail"] = detail ?? "";
        payload["updated_at"] = DateTimeOffset.Now.ToString("o");
        payload["cooldown_until"] = cooldownUntil == DateTime.MinValue ? "" : new DateTimeOffset(cooldownUntil).ToString("o");
        payload["automation_running"] = _automationRunning != 0;
        try
        {
            Directory.CreateDirectory(Path.GetDirectoryName(RuntimeStatePath()));
            File.WriteAllText(RuntimeStatePath(), Json.Serialize(payload), new UTF8Encoding(false));
        }
        catch { }

        if (!String.Equals(previous, state, StringComparison.Ordinal))
            Log("状态切换：" + previous + " -> " + state + (String.IsNullOrWhiteSpace(detail) ? "" : "；" + detail));
    }
    ...
    private static int SelfTest()
    {
        ...
        if (config.trigger_cooldown_ms.GetValueOrDefault() != DefaultTriggerCooldownMs) return 13;
        config.automation_enabled = true;
        bool unsafeConfigRejected = false;
        try { ValidateConfig(config, true); }
        ...
        if (!HasClickSteps(captured)) return 14;
        if (HasClickSteps(new Config { click_steps = new ClickStep[0] })) return 15;
