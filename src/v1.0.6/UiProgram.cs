...
internal sealed class MainForm : Form
{
    ...
    private const string MonitorExe = "napcat-monitor-v1.0.6.exe";
    ...
    private void DetectGameResolution()
    {
        ...
        try
        {
            var start = new ProcessStartInfo(exe, "--detect-resolution \"" + processName.Replace("\"", "") + "\"")
            {
            ...
            using (Process process = Process.Start(start))
            {
                ...
                process.WaitForExit();
                Match match = Regex.Match(output, "(\\d+)x(\\d+)");
                if (process.ExitCode != 0 || !match.Success)
                {
                    MessageBox.Show(String.IsNullOrWhiteSpace(error) ? "读取失败：" + output : error, "读取失败", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
    ...
    {
        if (config.click_steps == null || config.click_steps.Count == 0) return;
    ...
    private bool LoadCapturedPointsToUi(bool showErrors)
    {
        ...
        try
        {
            ...
            if (captured == null || captured.click_steps == null || captured.click_steps.Count == 0) return false;
